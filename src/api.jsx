...
export const getArticles = async (sort, topic, page, limit) => {
  if (sort === undefined) {
    if (topic === undefined) {
      const { data } = await axios.get(`${BASE_URL}/articles?p=${page}&limit=${limit}`);
      return data.articles;
    }
    const { data } = await axios.get(
      `${BASE_URL}/topics/${topic}/articles?p=${page}&limit=${limit}`,
    );
    return data.articles;
  }
  if (sort !== undefined) {
    const { sort_by, sort_ascending } = sort;
    let query = `?limit=${limit}&sort_by=${sort_by}&p=${page}`;
    ...
    const { data } = await axios.get(`${BASE_URL}/topics/${topic}/articles${query}`);
    return data.articles;
  }
};
...
export const articleLimits = async (limit, topic) => {
  ...
  const { data } = await axios.get(`${BASE_URL}/topics/${topic}/articles?limit=${limit}`);
  return data.articles;
};
...
export const articleSortBy = async (sort, topic) => {
  ...
  if (topic === undefined) {
    const { data } = await axios.get(`${BASE_URL}/articles?sort_by=${sort.sort_by}${ascStr}`);
    return data.articles;
  }
  const { data } = await axios.get(
...
export const getArticlesMostComments = async () => {
  const { data } = await axios.get(`${BASE_URL}/articles?sort_by=comment_count`);
  return data.articles;
};
...
export const postArticle = async (newArticle) => {
  const { data } = await axios.post(`${BASE_URL}/topics/${newArticle.topic}/articles`, {
...
export const postCommentOnArticle = async (newComment) => {
  const { data } = await axios.post(`${BASE_URL}/articles/${newComment.article_id}/comments`, {
...
export const deleteItem = async (id) => {
  if (id.comment_id === undefined) {
    const { data } = await axios.delete(`${BASE_URL}/articles/${id.article_id}`);
    return data;
  }
...
export const vote = async (toPatch) => {
  ...
  if (comment_id === undefined) {
    const { data } = await axios.patch(`${BASE_URL}/articles/${toPatch.article_id}`, {
      inc_votes: inc,
    });
    return data;
  }
